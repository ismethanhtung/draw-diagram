...
import { awsIcons, AwsIconType } from '../data/aws-icons';
...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <div className={`fixed top-0 left-0 h-full z-20 transition-transform duration-300 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
      <div className={`w-64 h-full shadow-2xl border-r flex flex-col ${isDarkMode ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-zinc-100'}`}>
        
        {/* Sidebar Tabs */}
        <div className="flex border-b border-zinc-200 dark:border-zinc-800 mt-12">
            <button 
              onClick={() => setActiveTab('properties')}
              className={`flex-1 py-3 text-xs font-bold uppercase tracking-wider transition-colors ${activeTab === 'properties' ? 'text-indigo-600 border-b-2 border-indigo-600 bg-indigo-50/50 dark:bg-indigo-900/20' : 'text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-300'}`}
            >
              Properties
            </button>
            <button 
              onClick={() => setActiveTab('library')}
              className={`flex-1 py-3 text-xs font-bold uppercase tracking-wider transition-colors ${activeTab === 'library' ? 'text-indigo-600 border-b-2 border-indigo-600 bg-indigo-50/50 dark:bg-indigo-900/20' : 'text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-300'}`}
            >
              Library
            </button>
        </div>
                ...
                <div className="flex gap-1.5 flex-wrap">
                  {colors.map(color => (
                    <button
                      ...
                      className={`w-6 h-6 rounded-md border-2 border-zinc-200 dark:border-zinc-700 transition-all ${activeStrokeColor === color ? 'border-indigo-500 scale-110' : ''}`}
              ...
              <section>
                ...
                <div className="flex gap-1.5 flex-wrap">
                  <button
                    ...
                    className={`w-6 h-6 rounded-md border-2 border-zinc-200 dark:border-zinc-700 flex items-center justify-center relative bg-zinc-50 dark:bg-zinc-800 ${activeBackgroundColor === 'transparent' ? 'border-indigo-500' : ''}`}
                  >
                  ...
                  {colors.map(color => (
                    <button
                      ...
                      className={`w-6 h-6 rounded-md border-2 border-zinc-200 dark:border-zinc-700 transition-all ${activeBackgroundColor === color ? 'border-indigo-500 scale-110' : ''}`}
                      style={{ backgroundColor: color }}
                    />
                  ))}
                </div>
